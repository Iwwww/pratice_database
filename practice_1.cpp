...
int centred(const int, int);
int centred(const int, int, ostream&);
int centred(const char[], int);
...
void print_hat();
void output_row(Student data[], int);
...
int centred(const int output, int length, ofstream &out) {
	int len{0};
	int tmp = output;
	while (tmp != 0) {
		tmp /= 10;
...
int centred(const int output, int length) {
	int len{0};
	int tmp = output;
	while (tmp != 0) {
		tmp /= 10;
...
int centred(const char output[], int length) {
	int len = strlen(output);
	char spacing[30] = " ";
	for (int i = 1; i < length - len; i++) {
		spacing[i] = ' ';
...
void print_hat() {
	cout << setw(MAX_NAME_LEN + 1 + MAX_NAME_LEN + 1 + MAX_LAST_NAME_LEN + 1 + 13 + GROUP_WIDTH + 1 + 13) << "Оценки:" << endl;
	cout << "№ |";
	centred("Имя", MAX_NAME_LEN + 1);
	cout << "|";
	centred("Фамилия", MAX_SURNAME_LEN + 1);
	cout << "|";
	centred("Отчество", MAX_LAST_NAME_LEN + 1);
	cout << "|";
	centred("Год рождения", 13);
	cout << "|";
	centred("Группа", GROUP_WIDTH + 1);
	cout << "|";
	for (int i{}; i < MARK_COUNT; i++) {
		char str_subject[10] = "Предмет ";
		str_subject[8] = char(i + 1 + 48);
		centred(str_subject, 10);
		cout << "|";
	}
	cout << endl;
}

void output_row(Student data[], int index) {
	cout << setw(2) << right << index + 1 << "|";
	centred(data[index].name, MAX_NAME_LEN + 1);
	cout << "|";
	centred(data[index].sur_name, MAX_SURNAME_LEN + 1);
	cout << "|";
	if (strcmp(data[index].last_name, "") != 0 && strcmp(data[index].last_name, "-") != 0) {
		centred(data[index].last_name, MAX_LAST_NAME_LEN + 1);
	}
	else {
		centred(" ", MAX_LAST_NAME_LEN + 1);
	}
	cout << "|";
	if (data[index].year_of_birth != 0) {
		centred(data[index].year_of_birth, 13);
	}
	else {
		centred(" ", 13);
	}
	...
	if (data[index].group != 0) {
		centred(data[index].group, GROUP_WIDTH + 1);
	}
	else {
		centred(" ", GROUP_WIDTH + 1);
	}
	cout << "|";
	for (int i = 0; i < MARK_COUNT; i++) {
		if (data[index].marks[i] != 0) {
			centred(data[index].marks[i], 10);
		}
		else {
			centred(" ", 10);
		}
...
int delete_note(Student data[]) {
	...
	do {
		...
		if (data[index].name[0]) {
			...
			*/
			memset(&data[index], 0, sizeof(Student));
...
void output_on_request(Student data[]) {
	system("cls");
	cout << "===============================================================================" << endl;
	cout << "||ВЫВОД СТУДЕНТОВ, ПОЛУЧИВШИХ НЕ БОЛЕЕ ДВУХ 4 (ОСТАЛЬНЫЕ ОЦЕНКИ - 5) НА ЭКРАН||" << endl;
	cout << "===============================================================================" << endl;
...
int sort_year_of_birth_menu(Student data[]) {
	system("cls");
	cout << "=================================" << endl;
	cout << "|| СОРТИРОВКА ПО ГОДУ РОЖДЕНИЯ ||" << endl;
	cout << "=================================" << endl;
	cout << "1| Сверху старше" << endl;
	cout << "2| Снизу старше" << endl;
	cout << "0| Назад" << endl;
	cout << "Введите номер пункта меню: ";
...
int sort_group_menu(Student data[]) {
	system("cls");
	cout << "==========================" << endl;
	cout << "|| СОРТИРОВКА ПО ГРУППЕ ||" << endl;
	cout << "==========================" << endl;
...
int counvert_bin_file(Student data[], char in_filename[], char out_filename[]) {
	...
	while (!fin.eof()) {
		fin >> data[index].name;
		fin >> data[index].sur_name;
		fin >> data[index].last_name;
		fin >> data[index].group;
		fin >> data[index].year_of_birth;
		for (int i = 0; i < MARK_COUNT; i++) {
			fin >> data[index].marks[i];
		}
		/*
		...
		write(s, count)
		*/
		fout.write((char*)&data[index], sizeof(data[index]));
		index++;
	}
...
int convert_to_bin_file_menu(Student data[]) {
	system("cls");
	cout << "===============================================" << endl;
	cout << "ПЕРЕВОД ДАННЫХ ТЕКСТОВОГО ФАЙЛА В БИНАРНЫЙ ФАЙЛ" << endl;
	cout << "===============================================" << endl;
...
int read_text_file_menu(Student data[]) {
	system("cls");
	cout << "====================================================" << endl;
	cout << "ВВОД С ИНФОРМАЦИИ ИЗ УЖЕ СОЗДАННОГО ТЕКСТОВОГО ФАЙЛА" << endl;
	cout << "====================================================" << endl;
	cout << "Введите имя файла:";
	char filename[FILE_NAME_LEN + 4] = "";
	input_str(filename, FILE_NAME_LEN, "-_+0123456789()", 0);
...
int read_bin_file(Student data[], char filename[]) {
	...
	int index{};
	while (!fin.eof()) {
		fin.read((char*)&data[index], sizeof(data[index]));
		index++;
	}
	fin.close();
...
int read_bin_file_menu(Student data[]) {
	system("cls");
	cout << "===================================================" << endl;
	cout << "ВВОД С ИНФОРМАЦИИ ИЗ УЖЕ СОЗДАННОГО БИНАРНОГО ФАЙЛА" << endl;
	cout << "===================================================" << endl;
	cout << "Введите имя файла:";
...
int write_file_table(char filename[], Student data[], int rows) {
	ofstream fout(filename, ios_base::out | ios_base::trunc);
	if (!fout.is_open()) return 1;
	fout << setw(MAX_NAME_LEN + 1 + MAX_NAME_LEN + 1 + MAX_LAST_NAME_LEN + 1 + 13 + GROUP_WIDTH + 1 + 13) << "Оценки:" << endl;
	fout << "№ |";
	centred("Имя", MAX_NAME_LEN + 1, fout);
	fout << "|";
	centred("Фамилия", MAX_SURNAME_LEN + 1, fout);
	fout << "|";
	centred("Отчество", MAX_LAST_NAME_LEN + 1, fout);
	fout << "|";
	centred("Год рождения", 13, fout);
	fout << "|";
	centred("Группа", GROUP_WIDTH + 1, fout);
	fout << "|";
	for (int i{}; i < MARK_COUNT; i++) {
		char str_subject[10] = "Предмет ";
		str_subject[8] = char(i + 1 + 48);
		centred(str_subject, 10, fout);
		fout << "|";
	}
	fout << endl;
	/*
	fout << setw(MAX_NAME_LEN + 1 + MAX_NAME_LEN + 1 + MAX_LAST_NAME_LEN + 1 + 13 + GROUP_WIDTH + 1 + 8) << "Оценки:" << endl;
	fout << "№ |";
	fout << setw(MAX_NAME_LEN + 1) << left << "Имя";
	fout << "|";
	fout << setw(MAX_SURNAME_LEN + 1) << "Фамилия";
	fout << "|";
	fout << setw(MAX_LAST_NAME_LEN + 1) << "Отчество";
	fout << "|";
	fout << setw(13) <<  "Год рождения";
	fout << "|";
	fout << setw(GROUP_WIDTH + 1) << "Группа";
	fout << "|";
	for (int i{}; i < MARK_COUNT; i++) {
		char str_subject[10] = "Предмет ";
		str_subject[8] = char(i + 1 + 48);
		fout << setw(10) << str_subject;
		fout << "|";
	}
	*/
	for (int index = 0; index < rows; index++) {
		fout << setw(2) << right << index + 1 << "|";
		centred(data[index].name, MAX_NAME_LEN + 1, fout);
		fout << "|";
		centred(data[index].sur_name, MAX_SURNAME_LEN + 1, fout);
		fout << "|";
		if (strcmp(data[index].last_name, "") != 0 && strcmp(data[index].last_name, "-") != 0) {
			centred(data[index].last_name, MAX_LAST_NAME_LEN + 1, fout);
		}
		else {
			centred(" ", MAX_LAST_NAME_LEN + 1, fout);
		}
		fout << "|";
		if (data[index].year_of_birth != 0) {
			centred(data[index].year_of_birth, 13, fout);
		}
		else {
			centred(" ", 13, fout);
		}
		fout << "|";
		if (data[index].group != 0) {
			centred(data[index].group, GROUP_WIDTH + 1, fout);
		}
		else {
			centred(" ", GROUP_WIDTH + 1, fout);
		}
		fout << "|";
		for (int i = 0; i < MARK_COUNT; i++) {
			if (data[index].marks[i] != 0) {
				centred(data[index].marks[i], 10, fout);
			}
			else {
				centred(" ", 10, fout);
			}
...
int write_file_table_menu(Student data[]) {
	system("cls");
	cout << "===================================================" << endl;
	cout << "ПЕРЕВОД БАЗЫ ДАННЫХ В ВИДЕ ТАБЛИЦИ В ТЕКСТОВЫЙ ФАЙЛ" << endl;
	cout << "===================================================" << endl;
	cout << "Введите имя текстового файла:";
